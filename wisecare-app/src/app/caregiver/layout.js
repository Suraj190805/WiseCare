...
import {
  ...
  Pill, LogOut, Menu, X, Phone, Settings
} from 'lucide-react';
...
function CaregiverLayoutInner({ children }) {
  ...
  const getPageTitle = () => {
    ...
    return route ? route.label : 'Caregiver Dashboard';
  };
  ...
  const handleCallPatient = () => {
    setCalling(true);
    addToast(`📞 Calling ${patient.name}...`, 'info');
    setTimeout(() => {
      setCalling(false);
      addToast(`📱 Call connected with ${patient.name}`, 'success');
    }, 2000);
  };
    ...
    <div className="app-layout">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`} style={sidebarOpen ? { transform: 'translateX(0)' } : {}}>
        ...
        <div className="sidebar-user">
          <div className="sidebar-avatar" style={{ background: 'linear-gradient(135deg, var(--accent-purple), var(--primary))' }}>{user.avatar}</div>
          <div>
            <div className="sidebar-user-name">{user.name}</div>
            <div className="sidebar-user-role">Caregiver • {user.relation}</div>
          </div>
        </div>
      </aside>
...
export default function CaregiverLayout({ children }) {
  return (
    <ToastProvider>
      <CaregiverLayoutInner>{children}</CaregiverLayoutInner>
    </ToastProvider>
  );
}
